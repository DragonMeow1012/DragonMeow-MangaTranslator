...
let _debugOn = false;
let _imgDiag = "";
let _prefetchCount = 10;    // 頁碼翻譯：預先翻譯頁數（popup 可調，預設 10）
let _pageTranslatePrefetch = 10; // 整頁翻譯：預先翻譯頁數（popup 可調，預設 10）
let _prefetchToEnd = false;     // 頁碼翻譯：翻到完（一直往後直到 404）
let _pagePrefetchToEnd = false; // 整頁翻譯：翻到完
let _autoPersistEnabled = false; // 頁碼翻譯啟用中（持久化；換頁/重載後自動續抓，讓進度往後滑動）
let _pagePersistEnabled = false; // 整頁翻譯啟用中（持久化）
const PREFETCH_TO_END_CAP = 200; // 「翻到完」往後猜的上限（背景遇 404 會自然停）
let _showPrefetch = true;   // 顯示預抓頁數（指泡泡時的 tooltip）
let _lastPrefetchPage = 0;
let _pfProg = null;         // 頁碼翻譯預抓即時進度 {done,total}，顯示在泡泡 tooltip
function loadDebugFlag() {
  chromeSafeGet(["dmmtDebugInput", "dmmtPrefetchCount", "dmmtPrefetchNotify", "dmmtPageTranslatePrefetch", "dmmtPrefetchToEnd", "dmmtPagePrefetchToEnd"], (s) => {
    _debugOn = s?.dmmtDebugInput === true;
    if (typeof s?.dmmtPrefetchCount === "number") _prefetchCount = s.dmmtPrefetchCount;
    if (typeof s?.dmmtPageTranslatePrefetch === "number") _pageTranslatePrefetch = s.dmmtPageTranslatePrefetch;
    if (typeof s?.dmmtPrefetchToEnd === "boolean") _prefetchToEnd = s.dmmtPrefetchToEnd;
    if (typeof s?.dmmtPagePrefetchToEnd === "boolean") _pagePrefetchToEnd = s.dmmtPagePrefetchToEnd;
  ...
  try {
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (changes.dmmtPrefetchCount && typeof changes.dmmtPrefetchCount.newValue === "number") {
        _prefetchCount = changes.dmmtPrefetchCount.newValue;
      }
      if (changes.dmmtPageTranslatePrefetch && typeof changes.dmmtPageTranslatePrefetch.newValue === "number") {
        _pageTranslatePrefetch = changes.dmmtPageTranslatePrefetch.newValue;
      }
      if (changes.dmmtPrefetchToEnd && typeof changes.dmmtPrefetchToEnd.newValue === "boolean") {
        _prefetchToEnd = changes.dmmtPrefetchToEnd.newValue;
      }
      if (changes.dmmtPagePrefetchToEnd && typeof changes.dmmtPagePrefetchToEnd.newValue === "boolean") {
        _pagePrefetchToEnd = changes.dmmtPagePrefetchToEnd.newValue;
      }
...
function refreshBubbleTitle() {
  ...
  const autoOn = state.autoTargets.length > 0 || state.autoArming;
  if (_showPrefetch && autoOn) {
    if (_pfProg) {
      state.button.title = `頁碼翻譯中… ${_pfProg.done}/${_pfProg.total}`;
    } else {
      state.button.title = _lastPrefetchPage > 0
        ? `已預先翻譯到第 ${_lastPrefetchPage} 頁`
        : `頁碼翻譯啟用中，翻頁後會預抓後 ${_prefetchCount} 頁`;
    }
  } else {
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  _pfProg = (message.total > 0 && message.done < message.total)
    ? { done: message.done, total: message.total }
    : null; // 追上或全部結束就清掉，tooltip 回到「已預先翻譯到第 N 頁」
  refreshBubbleTitle();
});
...
function schedulePrefetchNextPages(originalSrc, countOverride) {
  ...
  const defaultCount = _prefetchToEnd ? PREFETCH_TO_END_CAP : _prefetchCount;
  const count = clamp(typeof countOverride === "number" ? countOverride : defaultCount, 0, PREFETCH_TO_END_CAP);
  if (count <= 0) return;
...
function findMainPageImage() {
  let rep = null, best = 0;
  for (const img of document.querySelectorAll("img")) {
    if (!pageImageTranslatable(img)) continue;
    const area = (img.naturalWidth || 0) * (img.naturalHeight || 0);
    if (area > best) { best = area; rep = img; }
  }
  return rep;
}

function schedulePageTranslatePrefetch() {
  const count = _pagePrefetchToEnd ? PREFETCH_TO_END_CAP : _pageTranslatePrefetch;
  if (!_pageTranslate || count <= 0) return;
  const rep = findMainPageImage();
  if (rep) schedulePrefetchNextPages(bestImageUrl(rep), count);
}

// 換頁/重新載入後，若頁碼翻譯或整頁翻譯仍啟用，從本頁主圖往後續抓，
// 讓「預先翻譯頁數」隨閱讀進度持續往後滑動（全重載站點靠這個維持，例如 nhentai）。
function resumePrefetchOnLoad() {
  chromeSafeGet(["dmmtAutoEnabled", "dmmtAutoOrigin", "dmmtPrefetchCount", "dmmtPrefetchToEnd",
                 "dmmtPageEnabled", "dmmtPageOrigin", "dmmtPageTranslatePrefetch", "dmmtPagePrefetchToEnd"], (s) => {
    ...
    const autoCount = s?.dmmtPrefetchToEnd ? PREFETCH_TO_END_CAP
                      : (typeof s?.dmmtPrefetchCount === "number" ? s.dmmtPrefetchCount : 10);
    const pageCount = s?.dmmtPagePrefetchToEnd ? PREFETCH_TO_END_CAP
                      : (typeof s?.dmmtPageTranslatePrefetch === "number" ? s.dmmtPageTranslatePrefetch : 10);
    const count = Math.max(_autoPersistEnabled ? autoCount : 0, _pagePersistEnabled ? pageCount : 0);
    if (count > 0) kickResumePrefetch(count, 0);
  });
}
...
function kickResumePrefetch(count, attempt) {
  const rep = findMainPageImage();
  if (rep && rep.complete && (rep.naturalWidth || 0) > 200) {
    schedulePrefetchNextPages(bestImageUrl(rep), count);
  } else if (attempt < 30) {
