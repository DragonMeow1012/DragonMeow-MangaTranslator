...
function refreshBubbleTitle() {
  ...
  if (_showPrefetch && autoOn) {
    state.button.title = _pfProg ? `頁碼翻譯 ${_pfProg.done}/${_pfProg.total}` : "頁碼翻譯 0/0";
  } else {
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  _pfProg = { done: message.done, total: message.total }; // 保留最新進度，泡泡 tooltip 一直顯示 n/n
  if (_showPrefetch && message.total > 0 && message.done < message.total) {
    showStatus(`頁碼翻譯中… ${message.done}/${message.total}`, 1600);
  }
  refreshBubbleTitle();
});
