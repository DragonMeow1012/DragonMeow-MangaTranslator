...
from .rendering.text_render import region_default_font as _region_default_font
...
def _bubble_render_sort_key(region):
    try:
...
def _join_bubble_translations(regions):
    parts = []
    for region in sorted(regions, key=_bubble_render_sort_key):
        text = (region.translation or '').strip()
        if not text:
            continue
        core = text.strip('…。、，,！？!? ')
        if core and any(core in part for part in parts):
            continue
        parts = [
            part for part in parts
            if not core or part.strip('…。、，,！？!? ') not in text
        ]
        parts.append(text)
    return _normalize_joined_bubble_text(''.join(parts))


def _normalize_joined_bubble_text(text: str) -> str:
    text = (text or '').strip()
    if not text:
        return text
    ...
    text = re.sub(r'(唔……唔……)唔……', r'\1', text)
    text = text.replace('！?', '！？').replace('!?', '！？')
    text = _polish_scanlation_phrases(text)
    return text


def _polish_scanlation_phrases(text: str) -> str:
    """Small deterministic polish pass for common machine-translationese."""
    replacements = (
        ('什麼喔啊', '才不是「喔」啦'),
        ('才不是喔啦', '才不是「喔」啦'),
        ('才不是喔', '才不是「喔」'),
...
def _polish_scanlation_region(region) -> None:
    text = (region.translation or '').strip()
    if not text:
        return
    source = (getattr(region, 'text_raw', '') or region.text or '').replace('．', '.')
    if 'おおじゃない' in source or 'おおじゃないわ' in source:
        text = '才不是「喔」啦！'
    region.translation = _polish_scanlation_phrases(text)


def _choose_render_style_region(regions):
    def score(region):
        try:
            area = int(region.xywh[2] * region.xywh[3])
        except Exception:
            area = 0
        return (1 if (region.translation or '').strip() else 0, area)

    return max(regions, key=score)


_FALSE_ENV_VALUES = ('0', 'false', 'False', 'no', 'NO', 'off', 'OFF')
_PRESERVE_SFX_ENABLE = os.environ.get('MANGA_TRANSLATOR_PRESERVE_SFX', '1') not in _FALSE_ENV_VALUES
_PRESERVE_SFX_MAX_CHARS = int(os.environ.get('MANGA_TRANSLATOR_PRESERVE_SFX_MAX_CHARS', '8'))
_PRESERVE_SFX_PROB_THRESH = float(os.environ.get('MANGA_TRANSLATOR_PRESERVE_SFX_PROB_THRESH', '0.82'))
_SFX_KANA_RE = re.compile(r'[\u3040-\u30ff\u31f0-\u31ffーｰ～〜…‥・っッ]')
_SFX_MOTIF_RE = re.compile(
    r'(ズキ|ずき|ドキ|どき|ビク|びく|ピク|ぴく|チク|ちく|ガン|ごん|ゴン|'
    r'ベシ|べし|バシ|ばし|バッ|ばっ|パッ|ぱっ|ギュ|ぎゅ|キュ|きゅ|'
    r'ザワ|ざわ|ジッ|じっ|ズ|ず|ド|ど|ガ|が|ゴ|ご|ビ|び|ピ|ぴ)'
)
...
def _is_likely_preserve_sfx(region) -> bool:
    if not _PRESERVE_SFX_ENABLE or region is None:
        return False

    src = (getattr(region, 'text_raw', '') or getattr(region, 'text', '') or '').strip()
    if not src:
        return False

    src_len = _compact_text_len(src)
    if src_len == 0 or src_len > _PRESERVE_SFX_MAX_CHARS:
        return False

    # Dialogue with CJK ideographs should still be translated; SFX OCR is mostly
    # kana/symbolic sound text.
    if any('\u4e00' <= ch <= '\u9fff' for ch in src):
        return False

    significant = [ch for ch in src if not ch.isspace()]
    kana_count = sum(1 for ch in significant if _SFX_KANA_RE.search(ch))
    kana_ratio = kana_count / max(len(significant), 1)
    if kana_ratio < 0.55:
        return False

    prob = float(getattr(region, 'prob', 1.0) or 1.0)
    in_bubble = (
        getattr(region, '_bubble_idx', -1) >= 0
        or getattr(region, '_layout_role', '') == 'dialogue'
        or getattr(region, '_bubble_rect', None) is not None
    )
    motif_hit = bool(_SFX_MOTIF_RE.search(src))
    ...
    small_in_bubble = True
    rect = getattr(region, '_bubble_rect', None)
    if rect is not None:
        try:
            bx1, by1, bx2, by2 = [int(v) for v in rect]
            bubble_area = max(1, (bx2 - bx1) * (by2 - by1))
            region_area = max(1, int(region.xywh[2] * region.xywh[3]))
            small_in_bubble = region_area / bubble_area <= 0.22
        except Exception:
            small_in_bubble = True

    return small_in_bubble and motif_hit and (expressive_marks or prob <= 0.92)


def _drop_preserved_sfx_regions(text_regions):
    kept = []
    dropped = 0
    for region in text_regions:
        if _is_likely_preserve_sfx(region):
            region._preserve_original = True
            dropped += 1
            logger.info(f'[SFX] preserve original, skip translate/inpaint: {getattr(region, "text", "")!r}')
            continue
        kept.append(region)
    if dropped:
        logger.info(f'[SFX] preserved {dropped} original SFX regions')
    return kept


# 任何假名/漢字/諺文/拉丁字母/數字（含全形）都算「有內容」；都沒有 = 純符號
_HAS_WORD_CHAR_RE = re.compile(r'[0-9A-Za-z０-９Ａ-Ｚａ-ｚぁ-ゖァ-ヺｦ-ﾝ一-鿿가-힣]')


def _is_pure_symbol(text: str) -> bool:
    text = (text or '').strip()
    return bool(text) and not _HAS_WORD_CHAR_RE.search(text)


def _record_dropped_blank_lines(ctx, region) -> None:
    """記下「空譯文被剔除」region 的 textline，供 mask refinement 挖洞保留原字。

    Context 對缺失屬性回 None（不 raise），不能用 hasattr 判斷。
    """
    if ctx is None:
        return
    if getattr(ctx, '_dropped_blank_lines', None) is None:
        ctx._dropped_blank_lines = []
    try:
        ctx._dropped_blank_lines.extend(np.asarray(l) for l in region.lines)
    except Exception:
        pass


def _coalesce_translated_bubble_regions(text_regions, ctx=None):
    """逐 region 渲染；同泡合併已棄用。

    舊版會把同一顆泡泡的多個 region 併成一個渲染框，但相鄰 region 一併進來就把
    剛好塞滿泡泡的譯文推出框（實例：咪咪句 + 誤切碎片「い」出框）。使用者裁定棄用。
    保留 polish + 泡內 region 標 dialogue role（供 SFX 判定與 renderer 使用）。
    """
    for region in text_regions:
        _polish_scanlation_region(region)
        if getattr(region, '_bubble_idx', -1) >= 0 and getattr(region, '_bubble_rect', None) is not None:
...
class _PriorityGpuLock:
    """
    GPU 互斥 lock，post 階段優先於 pre 階段。
    用法：
        async with lock.acquire('pre'): ...   # 普通優先
        async with lock.acquire('post'): ...  # 高優先（插隊）

    為什麼要 priority：K 張並行進來時，K 個 pre 同時搶鎖，FIFO 下後到的 post-llm 會排在
    所有 pre 後面，導致「先把 K 張 pre 全做完才開始 post」，沒 overlap。post 永遠插隊
    確保「已經做一半的圖優先收尾」。
    """
    def __init__(self):
        self._held = False
        self._post_waiters: list = []
        self._pre_waiters: list = []

    def acquire(self, kind: str):
        return _GpuLockCtx(self, kind)

    async def _acquire(self, kind: str):
        assert kind in ('pre', 'post')
        # post 進來：只要沒人持鎖就拿；pre 還要額外條件「沒 post 在等」
        if not self._held and (kind == 'post' or not self._post_waiters):
            self._held = True
            return
        fut = asyncio.get_event_loop().create_future()
        (self._post_waiters if kind == 'post' else self._pre_waiters).append(fut)
        try:
            await fut
        except asyncio.CancelledError:
            # 兩種 race：
            # 1. fut 還在 list（release 還沒到）→ 移除即可，沒拿到 lock
            # 2. fut 已被 release pop 並 set_result（隱式擁有 lock），但 cancel 也來了
            #    → 必須 release 給下一個 waiter，否則 _held=True 永遠卡住
            if fut.done() and not fut.cancelled():
                # 被 release 設過 result，等於我們短暫擁有 lock，要 release 出去
                self._release()
            else:
                for lst in (self._post_waiters, self._pre_waiters):
                    if fut in lst:
                        lst.remove(fut)
            raise

    def _release(self):
        # 派發優先 post，沒 post 才輪到 pre
        if self._post_waiters:
            self._post_waiters.pop(0).set_result(None)
            return
        if self._pre_waiters:
            self._pre_waiters.pop(0).set_result(None)
            return
        self._held = False


class _GpuLockCtx:
    """async context manager wrapper for _PriorityGpuLock。"""
    def __init__(self, lock: _PriorityGpuLock, kind: str):
        self._lock = lock
        self._kind = kind
...
class MangaTranslator:
    ...
    async def _translate(self, config: Config, ctx: Context) -> Context:
        """
        完整 pipeline。三個 stage 之間靠 self._gpu_lock 序列化 GPU 階段、LLM 階段不持鎖。

        排程效果（K 張並行進來）：
          頁1 GPU(pre) → 頁1 LLM        → 頁1 GPU(post) → 頁1 完成
          頁2          → 頁2 GPU(pre) → 頁2 LLM        → 頁2 GPU(post)
          頁3                          → 頁3 GPU(pre) → 頁3 LLM ...

        gpu_lock 用 priority：post 永遠插隊 pre，避免「30 張 pre 排在前面、第 1 張的 post
    ...
    def _get_gpu_lock(self):
        """Lazy 建構：__init__ 時 event loop 還沒起來，asyncio.Lock 一定要在 loop 內建。"""
        lk = getattr(self, '_gpu_lock', None)
        if lk is None:
            lk = _PriorityGpuLock()
            self._gpu_lock = lk
        return lk
    ...
    async def _stage_pre_llm(self, config: Config, ctx: Context) -> Context:
        ...
        # 把 self._current_image_context 快照進 ctx；K 並發下 self.* 會被別張覆蓋，
        # _stage_post_llm 不可直接讀 self._current_image_context。
        # （legacy translate() 在這之前已呼叫 _set_image_context，self.* 暫時是這張的）
        if getattr(ctx, 'image_context', None) is None and self._current_image_context:
    ...
    async def _stage_llm(self, config: Config, ctx: Context) -> Context:
        """
        LLM stage：純網路呼叫翻譯 backend，無 GPU 使用。
        失敗或無 region → ctx._pipeline_done=True，ctx.result 已 set。
        """
        # -- Translation
    ...
    async def _stage_post_llm(self, config: Config, ctx: Context) -> Context:
        ...
        # 在 rendering 狀態後送 folder 訊息。讀 ctx.image_context（_stage_pre_llm 已 snapshot），
        # 而非 self._current_image_context — K 並發下 self.* 會被別張覆蓋。
        ctx_img_ctx = getattr(ctx, 'image_context', None) or self._current_image_context
        if hasattr(self, '_progress_hooks') and ctx_img_ctx:
            folder_name = ctx_img_ctx['subfolder']
    ...
    # ===== Pipeline-mode public stage entries =====
    # 給 worker (mode/share.py) 透過 RPC 拆 stage 呼叫用。
    # 跨 stage 的 ctx 透過 worker 端 job dict 保存，序列化由 share.py 處理（pickle）。
    # 注意：pipeline 模式下因為多張並行交錯，跨頁上下文 (all_page_translations) 不應 append；
    # 由呼叫端 (server/main.py) 透過 skip_context_save 控制。

    async def translate_pre_llm(self, image: Image.Image, config: Config) -> Context:
        """Pipeline mode stage 1: 從 raw image 跑到 LLM 之前。"""
        ctx = Context()
        ctx.input = image
        ctx.result = None
        ctx.verbose = self.verbose
        self._set_image_context(config, image)
        ctx.debug_folder = self._get_image_subfolder()
        # 把 image_context snapshot 進 ctx；pipeline 模式下多個 stage 交錯時 self._current_image_context
        # 會被別張圖蓋掉，post-llm 必須能從 ctx 還原而不是讀 self.* 全域。
        ctx.image_context = dict(self._current_image_context) if self._current_image_context else None
        # 模型預載（同 translate() 開頭）；冷啟動時這裡也會花時間。
        if self.models_ttl == 0:
            if config.upscale.upscale_ratio:
                await prepare_upscaling(config.upscale.upscaler)
            await prepare_detection(config.detector.detector)
            ...
            if config.colorizer.colorizer != Colorizer.none:
                await prepare_colorization(config.colorizer.colorizer)
        return await self._stage_pre_llm(config, ctx)

    async def translate_llm(self, ctx: Context, config: Config) -> Context:
        """Pipeline mode stage 2: LLM 翻譯。"""
        if getattr(ctx, '_pipeline_done', False):
            return ctx
        return await self._stage_llm(config, ctx)

    async def translate_post_llm(self, ctx: Context, config: Config) -> Context:
        """Pipeline mode stage 3: mask/inpaint/render。"""
        if getattr(ctx, '_pipeline_done', False):
            return ctx
        # 還原 image_context（避免 _stage_post_llm 內讀到別張圖正在跑 pre-llm 時設的 self._current_image_context）
        saved = ctx.get('image_context') if isinstance(ctx, dict) else getattr(ctx, 'image_context', None)
        if saved:
            self._current_image_context = saved
        return await self._stage_post_llm(config, ctx)
    ...
    # cuDNN handle 是 per-thread 建的；模型在 thread A 載入，thread B 跑 forward 偶發會 hang
    # （特別 lama_mpe inpainter）。固定 thread → CUDA context 一次建好，後續 forward 重用。
    # gpu_lock 保證同一時間只有 1 個 op 進來，max_workers=1 不會競爭。
    _gpu_executor = None

    @classmethod
    def _ensure_gpu_executor(cls):
        if cls._gpu_executor is None:
            import concurrent.futures
            cls._gpu_executor = concurrent.futures.ThreadPoolExecutor(
                max_workers=1, thread_name_prefix='manga-gpu',
            )
        return cls._gpu_executor

    @classmethod
    async def _run_async_in_thread(cls, coro_func, *args, **kwargs):
        """
        在 dedicated GPU thread + 獨立 event loop 跑 async GPU 函式。
        torch.forward() 是 sync call，跑時 worker 主 event loop 會被卡住，包進 thread 後
        main loop 仍可推進（LLM 結果送回 to_thread callback、其他 stage 的 gpu_lock 釋放）。
        """
        def _runner():
    ...
    async def _run_colorizer(self, config: Config, ctx: Context):
        ...
        return await self._run_async_in_thread(
    ...
    async def _run_upscaling(self, config: Config, ctx: Context):
        ...
        result = await self._run_async_in_thread(
            dispatch_upscaling,
    ...
    async def _run_detection(self, config: Config, ctx: Context):
        ...
        result = await self._run_async_in_thread(
            dispatch_detection,
            config.detector.detector, ctx.img_rgb, config.detector.detection_size, config.detector.text_threshold,
    ...
    async def _run_textline_merge(self, config: Config, ctx: Context):
        ...
        for region in text_regions:
            ...
            if not inside_bubble:
                # 正式指派（_bubble_rect）要求重疊 35%／中心命中，小 blob（雪人泡泡的
                # 點點格）常落選。放寬：中心落在任一偵測泡泡框（+10% pad）內就算泡內，
                # 讓「……み」這類被 OCR 讀成純點點的泡內字保留給 vision 修正。
                rects = getattr(region, '_bubble_rects', None) or []
                try:
                    bx1, by1, bx2, by2 = [int(v) for v in region.xyxy]
                    cx, cy = (bx1 + bx2) / 2, (by1 + by2) / 2
                    for rx1, ry1, rx2, ry2 in rects:
                        px = max(6, int((rx2 - rx1) * 0.1))
                        py = max(6, int((ry2 - ry1) * 0.1))
                        if rx1 - px <= cx <= rx2 + px and ry1 - py <= cy <= ry2 + py:
                            inside_bubble = True
                            break
                except Exception:
                    pass
            not_valuable = not is_valuable_text(region.text)
            # Short in-bubble shouts can be mostly punctuation in OCR (っーっ！！！？).
            # Keep them for the vision translator instead of leaving the original text behind.
            if inside_bubble and not_valuable and len(region.text.strip()) <= 12:
                not_valuable = False
    ...
    async def _run_text_translation(self, config: Config, ctx: Context):
        ...
        ctx.text_regions = _drop_preserved_sfx_regions(ctx.text_regions)
        if not ctx.text_regions:
            return []
        ...
        ctx.text_regions = _coalesce_translated_bubble_regions(ctx.text_regions, ctx)
        ...
        for region in ctx.text_regions:
            ...
            if not region.translation.strip():
                ...
                elif _is_pure_symbol(region.text) and _is_pure_symbol(region.translation):
                    # 原文與譯文都只有符號（…／！？／♥ 等）：重畫同一串符號
                    # 只會改變字級毀排版 → 不翻、保留原圖
                    # （譯文有實字代表 vision 從泡內補讀到內容，照常渲染）
                    should_filter = True
                    filter_reason = "Pure-symbol region (keep original art)"
                elif (
                    getattr(region, '_layout_role', '') != 'dialogue'
                    and getattr(region, '_bubble_rect', None) is None
                    and not re.search(r'[一-鿿]', region.translation)
                ):
                    # 框外 region 譯文不含任何漢字 = LLM echo 原文／純假名殘留
                    # （大型藝術 SFX「ガチャ」被 OCR 殘片 echo 的實例）。
                    # 渲染只會把原作美術字擦掉換成醜版日文 → 剔除 + 挖洞保留原圖。
                    # 泡泡內不適用：回填原文照舊渲染（有字比空白好）。
                    should_filter = True
                    filter_reason = "Non-Chinese echo outside bubble (keep original art)"
                elif (
                    getattr(region, '_layout_role', '') != 'dialogue'
                    and getattr(region, '_bubble_rect', None) is None
                    and abs(float(getattr(region, 'angle', 0) or 0)) > 25
                ):
                    # 框外 + 傾角 >25° = 斜寫手寫裝飾字（哭喊、囈語）。翻譯後斜插
                    # 畫面會毀版面（「往這裡的」斜貼實例），且這類字是畫的一部分
            ...
            if should_filter:
                if region.translation.strip():
                    ...
                    if "keep original art" in filter_reason:
                        # 同空譯文：從 mask 挖掉、保留原作美術字
                        _record_dropped_blank_lines(ctx, region)
                else:
                    # 空譯文（翻譯端判定保留原圖的 SFX 等）：記下 textline，
                    # mask refinement 後從 mask 挖掉、保留原字（見 _run_mask_refinement）。
                    _record_dropped_blank_lines(ctx, region)
            else:
                new_text_regions.append(region)
    ...
    async def _run_mask_refinement(self, config: Config, ctx: Context):
        # cv2 morph ops 是 sync blocking，包進 dedicated GPU thread 才不會卡主 event loop
        mask = await self._run_async_in_thread(
            dispatch_mask_refinement,
        ...
        # 把「空譯文被剔除」的 region textline 從 mask 挖掉。complete_mask 會把
        # 距離 kept textline < 0.5 字級的 raw mask 成分一併撿走（零重疊也撿），
        # 相鄰泡泡裡被剔除的字會被誤擦成空框框——挖掉這些區域讓原字保留。
        dropped = getattr(ctx, '_dropped_blank_lines', None)
        if mask is not None and dropped:
            hole = np.zeros_like(mask)
            for pts in dropped:
                try:
                    cv2.fillPoly(hole, [np.round(pts).astype(np.int32)], 255)
                except Exception:
                    continue
            # 跟 complete_mask 的 dilation 同步外擴，否則殘留一圈暈邊
            pad = max(3, int(config.mask_dilation_offset or 0) + self.kernel_size * 2)
            hole = cv2.dilate(hole, np.ones((pad, pad), np.uint8))
            # 保留 kept region 的 textline 範圍不被挖：空泡補抓的合成 textline
            # 可能覆蓋整顆泡，泡內真字（歸泡失敗、另行合併的 region）會被
            # 一起挖掉導致原字沒 inpaint（手寫泡譯文疊原文的實例）
            ...
            for region in ctx.text_regions:
                ...
                for line in lines:
                    try:
                        cv2.fillPoly(protect, [np.round(np.asarray(line)).astype(np.int32)], 255)
                    except Exception:
                        continue
            protect = cv2.dilate(protect, np.ones((pad, pad), np.uint8))
            hole[protect > 0] = 0
            mask[hole > 0] = 0
            logger.info(f'[mask] 挖掉 {len(dropped)} 條空譯文 textline（保留原字、不 inpaint）')
        return mask
    ...
    async def _run_inpainting(self, config: Config, ctx: Context):
        ...
        return await self._run_async_in_thread(
            dispatch_inpainting,
            config.inpainter.inpainter, ctx.img_rgb, ctx.mask, config.inpainter,
    ...
    async def _run_text_rendering(self, config: Config, ctx: Context):
        ...
        # 無顯式字型時，依目標語言挑地區字形（簡中→SC、繁中→TC），
        # 避免 Noto .ttc 預設 JP 面把中文渲染成日文字形。env 指定字型仍優先。
        fallback_font_path = (
            config.render.font_path
            or self.font_path
            or _region_default_font(config.translator.target_lang)
        )
        if ctx.text_regions:
            for region in ctx.text_regions:
        ...
        if config.render.renderer == Renderer.none:
            ...
            # 保留單一 CJK renderer，但把 manga2eng / UI 風格映射成 balloon_fill preset，
            # 讓 bot 端仍能用 renderer/layout_mode 切換較貼近氣泡的排版策略。
