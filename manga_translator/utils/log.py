...
class Formatter(logging.Formatter):
    ...
    # Formatter.usesTime() 會看 _fmt 是否含 %(asctime)；但我們在 formatMessage 裡才動態
    # 換 _fmt，usesTime 早已決定不算 asctime → record.asctime 不存在 → KeyError。
    # 強制 usesTime=True，並在 super().format() 階段就會幫 record 算好 asctime。
    def usesTime(self) -> bool:
        return True

    def formatMessage(self, record: logging.LogRecord) -> str:
        if record.levelno >= logging.ERROR:
            self._style._fmt = f'%(asctime)s {colorama.Fore.RED}%(levelname)s:{colorama.Fore.RESET} [%(name)s] %(message)s'
        elif record.levelno >= logging.WARN:
            self._style._fmt = f'%(asctime)s {colorama.Fore.YELLOW}%(levelname)s:{colorama.Fore.RESET} [%(name)s] %(message)s'
