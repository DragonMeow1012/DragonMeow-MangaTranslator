...
def _resolve_manga_ocr_model_path() -> str:
    configured = os.environ.get('MANGA_OCR_MODEL_DIR', '').strip()
    if configured:
        return configured
    ...
    # 隨 release zip 附帶的本地模型（解壓即用，免額外下載）
    bundled = Path(__file__).resolve().parents[2] / 'models' / 'manga-ocr-base'
    if bundled.is_dir() and required.issubset({item.name for item in bundled.iterdir()}):
        return str(bundled)
    ...
    if cache_root.is_dir():
        ...
        for snapshot in cache_root.iterdir():
            if not snapshot.is_dir():
                continue
            if required.issubset({item.name for item in snapshot.iterdir()}):
