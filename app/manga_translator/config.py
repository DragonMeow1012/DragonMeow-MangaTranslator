...
class RenderConfig(BaseModel):
    ...
    """Force renderer to strictly adhere to the bounding box."""
    alignment: Alignment = Alignment.auto
    """Align rendered text"""
    disable_font_border: bool = False
    """Disable font border"""
    disable_auto_wrap: bool = False
    """Disable automatic line wrapping when the renderer supports it."""
    ...
    """Retry AI line-breaking if expected [BR] tags are missing."""
    strict_smart_scaling: bool = False
    """Keep smart scaling conservative when bubble fill cannot be used."""
    ...
    """Use bubble-aware horizontal layout for English text."""
    font_path: Optional[str] = None
    """Optional font path used as the default render font."""
    ...
    """Letter spacing multiplier relative to the font advance width."""
    ...
    """Rotate repeated punctuation marks in vertical text when appropriate."""
    layout_mode: Literal["fit", "balloon_fill", "smart_scaling", "strict"] = "fit"
    """Typesetting strategy. balloon_fill tries to use more of the detected bubble safe area."""
    stroke_width: float = 0.07
    """Stroke/border width ratio relative to font size."""
    enable_template_alignment: bool = False
    """Reserved compatibility flag for replace-translation mode."""
    paste_mask_dilation_pixels: int = 10
    """Reserved compatibility flag for replace-translation mode."""
    ai_renderer_concurrency: int = 1
    """Reserved compatibility flag for API renderers."""
    _font_color_fg = None
    _font_color_bg = None
    @property
...
class TranslatorConfig(BaseModel):
    translator: Translator = Translator.gemini_2stage
    """Language translator to use"""
    target_lang: str = 'ENG' #todo: validate VALID_LANGUAGES #todo: convert to enum
    """Destination language"""
    llm_provider: Optional[str] = None
    """LLM provider: gemini (default) / openai / deepseek / custom (OpenAI-compatible)."""
