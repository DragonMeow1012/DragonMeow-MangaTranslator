...
def fg_bg_compare(fg, bg):
    """Keep a readable foreground/background pair when OCR colors are unreliable."""
    fg_avg = np.mean(fg)
...
def _resolve_font_path(font_path: str) -> str:
    ...
    if not os.path.isabs(font_path):
        ...
        if os.path.exists(candidate):
            return candidate
        # 相對 fonts/ 目錄（使用者上傳字型存在 fonts/user/<name>）
...
def _build_region_reference_mask(
    ...
    # region 過半面積在泡內才算「在泡內」——任一像素重疊就置中會把
    # 框外手寫字（box 恰好掃過鄰泡）整句拉進別人的對話泡
    region_area = int(np.count_nonzero(region_mask))
    if region_area == 0 or int(np.count_nonzero(overlap_pixels)) < region_area * 0.5:
...
_BUBBLE_LOBE_CENTERING_ENABLE = os.environ.get('MANGA_TRANSLATOR_BUBBLE_LOBE_CENTERING', '1') not in _FALSE_ENV_VALUES


def _region_overlap_label_counts(
    region: TextBlock,
    bubble_mask: np.ndarray,
    label_map: Optional[np.ndarray],
) -> dict:
    if bubble_mask is None or label_map is None or label_map.shape[:2] != bubble_mask.shape[:2]:
        return {}
    ...
    overlap = (region_mask > 0) & (bubble_mask > 0)
    if not np.any(overlap):
        return {}

    labels, counts = np.unique(label_map[overlap], return_counts=True)
    return {int(label): int(count) for label, count in zip(labels, counts) if int(label) > 0}


def _bubble_mask_centroid(mask: np.ndarray) -> Optional[Tuple[float, float]]:
...
def _partition_component_mask_by_regions(
    component_mask: np.ndarray,
    regions: List[TextBlock],
) -> List[np.ndarray]:
    if component_mask is None or np.count_nonzero(component_mask) == 0 or not regions:
        return []
    if len(regions) == 1:
        return [np.where(component_mask > 0, 255, 0).astype(np.uint8)]

    ys, xs = np.nonzero(component_mask > 0)
    if xs.size == 0:
        return [np.zeros_like(component_mask, dtype=np.uint8) for _ in regions]

    x_min, x_max = float(xs.min()), float(xs.max())
    y_min, y_max = float(ys.min()), float(ys.max())
    scale_x = max(x_max - x_min, 1.0)
    scale_y = max(y_max - y_min, 1.0)

    centers = []
    for region in regions:
        cx, cy = np.asarray(region.center, dtype=np.float32).tolist()
        centers.append(((float(cx) - x_min) / scale_x, (float(cy) - y_min) / scale_y))
    centers_arr = np.asarray(centers, dtype=np.float32)

    px = ((xs.astype(np.float32) - x_min) / scale_x)[:, None]
    py = ((ys.astype(np.float32) - y_min) / scale_y)[:, None]
    dx = px - centers_arr[None, :, 0]
    dy = py - centers_arr[None, :, 1]
    nearest = np.argmin(dx * dx + dy * dy, axis=1)

    out = [np.zeros_like(component_mask, dtype=np.uint8) for _ in regions]
    for idx, mask in enumerate(out):
        owned = nearest == idx
        if np.any(owned):
            mask[ys[owned], xs[owned]] = 255
        else:
            # Degenerate tie or seed outside the component: keep a small local fallback
            # instead of disabling centering for this region.
            fallback = np.zeros_like(component_mask, dtype=np.uint8)
            cx, cy = np.asarray(regions[idx].center, dtype=np.int32).tolist()
            cv2.circle(fallback, (int(cx), int(cy)), max(4, int(min(scale_x, scale_y) * 0.08)), 255, -1)
            mask[:] = np.where((fallback > 0) & (component_mask > 0), 255, 0).astype(np.uint8)
    return out


def _prepare_bubble_layout_masks(
    ...
    for region in text_regions:
        if region is not None:
            region._bubble_layout_mask = None
            region._bubble_layout_center = None

    if (
        not _BUBBLE_LOBE_CENTERING_ENABLE
        or bubble_mask is None
        or label_map is None
        or label_map.shape[:2] != bubble_mask.shape[:2]
        or np.count_nonzero(bubble_mask) == 0
    ):
        return

    by_label: dict[int, List[Tuple[TextBlock, int]]] = {}
    for idx, region in enumerate(text_regions):
        if region is None or not getattr(region, 'translation', '').strip():
            continue
        counts = _region_overlap_label_counts(region, bubble_mask, label_map)
        if not counts:
            continue
        label = max(counts.items(), key=lambda item: item[1])[0]
        by_label.setdefault(label, []).append((region, idx))

    for label, indexed_regions in by_label.items():
        component_mask = np.where(label_map == label, 255, 0).astype(np.uint8)
        regions = [region for region, _ in indexed_regions]
        masks = _partition_component_mask_by_regions(component_mask, regions)
        for (region, _), layout_mask in zip(indexed_regions, masks):
            if layout_mask is None or np.count_nonzero(layout_mask) == 0:
                continue
            region._bubble_layout_mask = layout_mask
            region._bubble_layout_center = _bubble_mask_centroid(layout_mask)
...
def _clamp_center_drift(region: 'TextBlock', dst_points, max_ratio: float):
    """限制排版後字框中心相對「原 OCR 區域中心」的位移上限。

    balloon_fill 會把譯文置中到整個氣泡 → 原文若偏在氣泡一角，譯文中心會飄走。
    這裡把字框中心鎖在距原中心 ``max_ratio × (原區域寬, 高)`` 內，超過就整體平移拉回，
    不改字體大小／換行，只平移。env ``MANGA_RENDER_CENTER_DRIFT_RATIO`` 控制（<=0 關閉）。
    其他 layout 模式本來就用 region.center → 位移≈0，clamp 等於 no-op，故全模式套用安全。
    """
    if dst_points is None or max_ratio <= 0:
        return dst_points
    try:
        arr = np.asarray(dst_points, dtype=np.float32)
        cur = arr.reshape(-1, 2).mean(axis=0)
        orig = np.asarray(region.center, dtype=np.float32).reshape(2)
        mr = np.asarray(region.min_rect, dtype=np.float32).reshape(-1, 2)
        rw = float(np.linalg.norm(mr[1] - mr[0]))
        rh = float(np.linalg.norm(mr[2] - mr[1]))
        max_dx = max(max_ratio * max(rw, 1.0), 1.0)
        max_dy = max(max_ratio * max(rh, 1.0), 1.0)
        dx = float(cur[0] - orig[0])
        dy = float(cur[1] - orig[1])
        shift_x = dx - max(-max_dx, min(dx, max_dx))
        shift_y = dy - max(-max_dy, min(dy, max_dy))
...
def resize_regions_to_font_size(img: np.ndarray, text_regions: List['TextBlock'], config: Config, original_img: np.ndarray = None, return_debug_img: bool = False, skip_font_scaling: bool = False):
    ...
    # --- 短譯文防爆字：渲染字級 ≤ 原文字級 × MANGA_RENDER_MAX_FONT_SCALE（預設 1.4）---
    # 排版是「把字撐滿框」反推字級：短譯文（嗯／欸）配上大 OCR 框會被放成超大字。
    # 原文字級（偵測 textline 高度）是天然上限——原文多大、譯文就該差不多大。設 0 關閉。
    _max_scale = _safe_float(os.getenv('MANGA_RENDER_MAX_FONT_SCALE', '1.4'), 1.4)
    if _max_scale > 0:
        for _i, _region in enumerate(text_regions):
            if _region is None or _i >= len(dst_points_list) or dst_points_list[_i] is None:
                continue
            _base = int(getattr(_region, 'original_font_size', 0) or 0)
            if _base <= 0:
                continue
            _cap = max(8, int(_base * _max_scale))
            if _region.font_size > _cap:
                try:
                    _ratio = _cap / float(_region.font_size)
                    _arr = np.asarray(dst_points_list[_i], dtype=np.float32)
                    _c = _arr.reshape(-1, 2).mean(axis=0)
                    dst_points_list[_i] = (_arr - _c) * _ratio + _c
                    _region.font_size = _cap
                except Exception:
                    pass

    # --- 中心約束：限制譯文字框中心相對原 OCR 區域中心的位移（修 balloon_fill「字會飄」）---
    # env MANGA_RENDER_CENTER_DRIFT_RATIO 預設 0.6（=最多偏原區域 0.6 倍寬/高）；設 0 關閉。
    _drift_ratio = _safe_float(os.getenv('MANGA_RENDER_CENTER_DRIFT_RATIO', '0.6'), 0.6)
    if _drift_ratio > 0:
        for _i, _region in enumerate(text_regions):
            if _region is None or _i >= len(dst_points_list):
                continue
            dst_points_list[_i] = _clamp_center_drift(_region, dst_points_list[_i], _drift_ratio)
...
def _sample_region_text_color(region: 'TextBlock', original_img: np.ndarray):
    """看對話框底色決定字色：底亮 → 純黑、底暗 → 純白。

    OCR 顏色回歸不可靠（常回淺色 → 字跟泡泡底色融在一起、看起來像空白框），
    描邊又已關（disable_font_border）沒有對比兜底。改成最穩的二值方案：
    取區域灰階「中位數」當底色亮度（文字筆畫是少數像素，拉不動中位數），
    亮底配黑字、暗底配白字，對比永遠拉滿。彩色字不保留——可讀性優先。
    取樣失敗回 None（沿用 OCR 回歸色）。
    """
    try:
