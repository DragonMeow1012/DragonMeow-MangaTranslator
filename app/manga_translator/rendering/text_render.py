...
# Noto Sans CJK 擺第一：垂直標點（？！。」等）的 advance metric 正確，
# 直書時不會像台北黑體那樣擠進上一格。要完全比照 bot 嵌字效果，
# 在 .env 設 MANGA_TRANSLATOR_FONT_PATH 指向你的字型（如華康少女文字）即可覆蓋。
_BUNDLED_DEFAULT_FONT = os.path.join(BASE_PATH, 'fonts', 'NotoSansMonoCJK-VF.ttf.ttc')


def _resolve_default_font() -> str:
    """env MANGA_TRANSLATOR_FONT_PATH（絕對路徑或 fonts/ 內檔名）優先，否則用內建 Noto。"""
    env_font = (os.getenv('MANGA_TRANSLATOR_FONT_PATH') or '').strip()
    if env_font:
        cand = env_font if os.path.isabs(env_font) else os.path.join(BASE_PATH, 'fonts', env_font)
        if os.path.exists(cand):
            return cand
    return _BUNDLED_DEFAULT_FONT


DEFAULT_FONT = _resolve_default_font()
FALLBACK_FONTS = [
    DEFAULT_FONT,
    _BUNDLED_DEFAULT_FONT,
    os.path.join(BASE_PATH, 'fonts/TaipeiSansTCBeta-Regular.ttf'),
]

# 地區字形：Noto Sans Mono CJK 的 .ttc 預設用 JP 面，簡中/繁中會被日文字形渲染
...
# 波浪符與長音符同類：換成 ︴/≀ 字形會跟原作觀感差很多，旋轉原字才像嵌字。
