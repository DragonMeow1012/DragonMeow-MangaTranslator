...
**かんたん：[Chrome ウェブストア](https://chromewebstore.google.com/detail/gfoajcgelfgihpnaijmimhhfkioocoej) からワンクリックでインストール**（審査通過・公開済み）。インストール後：① 先にアプリを起動（`start.bat` / `bash start.sh`）し `http://127.0.0.1:8501` が開いていることを確認 → ② 拡張を開き「🔄 Web設定を取得」→ ③ 漫画ページで「✨ バブルを有効化」を押して開始。

ストアを使わない場合は、同梱の zip を**デベロッパーモード**で読み込むこともできます：
